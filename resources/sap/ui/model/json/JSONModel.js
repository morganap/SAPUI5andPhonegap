...
sap.ui.model.json.JSONModel=function(d){sap.ui.model.Model.apply(this,arguments);this.bCache=true;if(typeof d=="string"){this.loadData(d)}else if(d&&typeof d=="object"){this.setData(d)}};
sap.ui.model.json.JSONModel.prototype=jQuery.sap.newObject(sap.ui.model.Model.prototype);sap.ui.base.Object.defineClass("sap.ui.model.json.JSONModel",{baseType:"sap.ui.model.Model",publicMethods:["loadData","setData","getData","setJSON","getJSON","setProperty","forceNoCache"]});
...
sap.ui.model.json.JSONModel.prototype._getObject=function(p,c){var n=this.isLegacySyntax()?this.oData:null,i=p&&jQuery.sap.startsWith(p,"/")?false:true;if(c instanceof sap.ui.model.Context&&i){n=this._getObject(c.getPath());if(!n){return null}}else if(c){n=c}if(!p){return n}var P=p.split("/"),I=0;if(!P[0]){n=this.oData;I++}while(n&&P[I]){n=n[P[I]];I++}return n};
